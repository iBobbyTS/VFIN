...
def _make_video_dataset(dir):
    ...
    if '.DS_Store' in framesPath:
        framesPath.remove('.DS_Store')
    return framesPath
...
def _pil_loader(path, cropArea=None, resizeDim=None, frameFlip=0):
    ...
    wh = []
    origDim = img.shape[0:2]
    for i in (origDim[0] / 32, origDim[1] / 32):
        ans = i
        if i > int(i):
            ans = int(i) + 1
        wh.append(int(ans))
    dim = [wh[0] * 32, wh[1] * 32, 3]
    new = np.zeros(dim)
...
class Video(data.Dataset):
    ...
    def __init__(self, root, to_process, transform=None):
        """
        Parameters
        ----------
            root : string
