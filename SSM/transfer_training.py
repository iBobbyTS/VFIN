...
def print_and_log(log, *_args, **kwargs):
    print(log, *_args, **kwargs)
...
for epoch in range(args.epochs):
    ...
    scheduler.step()
    print_and_log('\rEpoch %d spent %.3f seconds. ' % (epoch, time.time() - epoch_start_time),
                  flush=True)
...
model_save_path = f'{os.path.splitext(args.checkpoint)[0]}_ep{args.epochs}_lr{args.learning_rate}.pth'
torch.save(dict1, model_save_path)
print_and_log(f'Saved transfer learned model to {model_save_path}.\n\n')
