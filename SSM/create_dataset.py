...
parser.add_argument("--img_width", type=int, default='', help="output image width")
parser.add_argument("--img_height", type=int, default='', help="output image height")
...
def extract_frames(videos, inDir, outDir):
    ...
    print(videos)
    for video in videos:
        if args.img_width == '' and args.img_width == '':
            scale = ''
        else:
            scale = f' -vf scale={args.img_width}:{args.img_height}'
        os.mkdir(os.path.join(outDir, os.path.splitext(video)[0]))
        retn = os.system(
            f'"{os.path.join(args.ffmpeg_dir, "ffmpeg")}" -i "{os.path.join(inDir, video)}" -vsync 0 "{os.path.join(outDir, os.path.splitext(video)[0])}/%04d.jpg"')
        print(f'Finished processing video {video}, {count}/{len(videos)} done.')
        count += 1
        if retn:
